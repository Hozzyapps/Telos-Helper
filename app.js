...
if (reader) reader.diffRead = false;   // read all visible lines; we de-dupe by text ourselves
...
function findTrigger(rawText) {
	...
	for (var i = 0; i < triggers.length; i++) {
		...
		for (var j = 0; j < phrases.length; j++) {
			if (text.indexOf(norm(phrases[j])) !== -1) return trig;
		}
	}
	return null;
}

/* Scan every visible line each tick and de-dupe by the exact line text, so a
   message that lingers on screen only fires once. On the first pass we mark
   what's already there as seen (no alert) so we don't fire on old chat. */
var seenLines = {};
var pruneCounter = 0;
function processLines(lines) {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		var dkey = norm(raw);
		if (!dkey) continue;
		var trig = findTrigger(raw);
		if (!trig) continue;
		if (seenLines[dkey]) continue;   // already handled this exact line
		seenLines[dkey] = now;
		if (primed) fireAlert(trig);     // during priming, mark seen but stay silent
	}
	primed = true;
	if (++pruneCounter % 40 === 0) {
		for (var k in seenLines) if (now - seenLines[k] > 300000) delete seenLines[k];
	}
}
